 ...
 * @param text
 */
export const ToBase64 = (text) => Buffer.from(text, 'utf-8').toString('base64');
/**
 * Convert base64 to a string
 *
 * @param base64
 */
export const FromBase64 = (base64) => Buffer.from(base64, 'base64').toString('utf-8');
/**
 ...
 * @returns
 */
export const stringIncludesArrayItems = (array, str) => array.some((item) => str.includes(item));
/**
 ...
 * @param str
 */
export const convertPathToArray = (str) => {
    if (str.includes('\\.')) {
        return str
            .replace(/\\\./g, '%#%')
            .split('.')
 ...
 * @returns
 */
export const routesFromFolder = (folderChildren, allFolders, allRoutes) => {
    const routesList = [];
    folderChildren.forEach((folderChild) => {
        if (folderChild.type === 'route') {
            ...
            if (subFolder) {
                routesList.push(...routesFromFolder(subFolder.children, allFolders, allRoutes));
            }
        }
    });
    return routesList;
 ...
 * @returns
 */
export const dedupSlashes = (str) => str.replace(/\/{2,}/g, '/');
