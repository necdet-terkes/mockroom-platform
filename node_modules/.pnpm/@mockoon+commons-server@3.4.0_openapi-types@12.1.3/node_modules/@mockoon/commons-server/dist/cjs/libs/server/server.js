...
class MockoonServer extends events_1.EventEmitter {
            ...
            request.on('end', () => {
                ...
                try {
                    if (requestContentType) {
                        ...
                        else if ((0, utils_1.stringIncludesArrayItems)(common_constants_1.ParsedXMLBodyMimeTypes, requestContentType)) {
    ...
    setRoutes(server) {
        ...
        const routes = (0, utils_1.routesFromFolder)(this.environment.rootChildren, this.environment.folders, this.environment.routes);
        routes.forEach((declaredRoute) => {
            ...
            if (declaredRoute.enabled) {
                try {
                    ...
                    routePath = (0, utils_1.dedupSlashes)(routePath);
    ...
    createRouteHandler(route, requestNumber, crudId) {
        return (request, response) => {
            ...
            if (!currentRoute) {
                this.sendError(response, commons_1.CommonsTexts.EN.MESSAGES.ROUTE_NO_LONGER_EXISTS, 404);
                return;
            }
            const enabledRouteResponse = new response_rules_interpreter_1.ResponseRulesInterpreter(currentRoute.responses, request, currentRoute.responseMode).chooseResponse(requestNumber);
            requestNumber += 1;
                    ...
                    if (enabledRouteResponse.bodyType === commons_1.BodyTypes.DATABUCKET &&
                        ...
                        if (servedDatabucket) {
                            ...
                            if (route.type === commons_1.RouteType.CRUD && crudId) {
                                content = (0, crud_1.databucketActions)(crudId, servedDatabucket, request, response);
                            }
    ...
    serveBody(content, routeResponse, request, response, templateParse = true) {
        try {
            if (!routeResponse.disableTemplating && templateParse) {
                content = (0, template_parser_1.TemplateParser)(false, content || '', this.environment, this.processedDatabuckets, request, response);
            }
            ...
            this.sendError(response, `${commons_1.CommonsTexts.EN.MESSAGES.ROUTE_SERVING_ERROR}: ${error.message}`);
        }
    }
    /**
    ...
    sendFile(routeResponse, routeContentType, request, response) {
        const fileServingError = (error) => {
            ...
            this.sendError(response, `${commons_1.CommonsTexts.EN.MESSAGES.ROUTE_FILE_SERVING_ERROR}: ${error.message}`);
        };
        ...
        try {
            let filePath = (0, template_parser_1.TemplateParser)(false, routeResponse.filePath.replace(/\\/g, '/'), this.environment, this.processedDatabuckets, request);
            ...
            if (commons_1.MimeTypesWithTemplating.indexOf(fileMimeType) > -1 &&
                ...
                (0, fs_1.readFile)(filePath, (readError, data) => {
                    ...
                    try {
                        const fileContent = (0, template_parser_1.TemplateParser)(false, data.toString(), this.environment, this.processedDatabuckets, request, response);
                        this.applyResponseLocals(response);
        ...
        if (this.environment.proxyMode &&
            ...
            server.use('*', (0, http_proxy_middleware_1.createProxyMiddleware)({
                ...
                onError: (error, request, response) => {
                    ...
                    this.sendError(response, `${commons_1.CommonsTexts.EN.MESSAGES.PROXY_ERROR}${this.environment.proxyHost}${request.url}: ${error}`, 504);
                }
            }));
        }
    }
    ...
    parseHeader(header, request) {
        ...
        if (header.key && header.value) {
            try {
                parsedHeaderValue = (0, template_parser_1.TemplateParser)(false, header.value, this.environment, this.processedDatabuckets, request);
            }
            catch (error) {
                const errorMessage = commons_1.CommonsTexts.EN.MESSAGES.HEADER_PARSING_ERROR;
        ...
        if (environment.data.length > 0) {
            environment.data.forEach((databucket) => {
                ...
                if (databucket.value.match(new RegExp(`{{2,3}[#(\\s\\w]*(${request_helpers_1.requestHelperNames.join('|')})`))) {
                    ...
                    try {
                        templateParsedContent = (0, template_parser_1.TemplateParser)(false, databucket.value, environment, this.processedDatabuckets);
            ...
            if (databucketIdsToParse.size > 0) {
                ...
                for (const databucketIdToParse of databucketIdsToParse) {
                    ...
                    if (targetDatabucket && !(targetDatabucket === null || targetDatabucket === void 0 ? void 0 : targetDatabucket.parsed)) {
                        ...
                        try {
                            content = (0, template_parser_1.TemplateParser)(false, targetDatabucket.value, environment, this.processedDatabuckets, request);
